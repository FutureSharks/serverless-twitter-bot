...
def run(api: object, options: dict, state: dict, recipient: str):
    ...
    plt.plot([round(random.uniform(-1, 1), 1)], [round(random.uniform(-1, 1), 1)], 'ro', markersize=40)
    plt.ylim(-1.1, 1.1)
    plt.xlim(-1.1, 1.1)
    plt.axhline(0, color='black', linewidth=4)
    plt.axvline(0, color='black', linewidth=4)
    r1 = plt.Rectangle((0,0), -1, 1, fc='red', alpha=0.3)
    r2 = plt.Rectangle((0,0), -1, -1, fc='green', alpha=0.3)
    r3 = plt.Rectangle((0,0), 1, 1, fc='blue', alpha=0.5)
    r4 = plt.Rectangle((0,0), 1, -1, fc='purple', alpha=0.3)
